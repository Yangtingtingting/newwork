...
<template>
    <div class="haveheaderbar">
        <headerbar :title="'收益'" :ifgocenter="false"></headerbar>
        <div class="top">
            <p>{{$route.query.invite_reward}} VPN</p>
        ...
        <div class="datalist">
            <div class="datali clearfix" v-for="(item,index) in 0" :key="index+'test'">
                <p>繁衍：1,234.88 VPN</p>
                <div class="right">
            ...
            <div class="datali clearfix" v-for="(item,index) in pagelist" :key="index">
                <p>{{item.type==1?'核聚变':item.type==2?'核裂变':item.type==3?'团队收益':item.type==4?'加速收益':''}}：{{item.amount}} VPN</p>
                <div class="right">
                    <div class="time">{{item.ctime}}</div>
                </div>
            </div>
            <!-- <div class="datali clearfix" v-if="pagelist.length==0" style="text-align:center;">
                暂无更多数据
            </div> -->
    ...
    export default {
        ...
        methods: {
            morebtnclick(){
                ...
                this.get_infoinviterecord();
            },
            // 分页获取奖励记录
            get_infoinviterecord(){
                let zs = this;
                this.$toast.loading({
                    duration:0,
                    forbidClick:true,
                })
                this.api.bussiness.get_infoinviterecord({
                    page:zs.pagenumber,
                    count:20,
                }).then(res=>{
                    zs.$toast.clear();
